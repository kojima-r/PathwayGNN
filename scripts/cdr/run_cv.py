#!/usr/bin/env python3
"""Run the drug-response cross-validation grid across the available GPUs.

Splits ``configs/cdr/cv.yaml`` into one config per (task, variant) and runs each
as ``pathwaygnn cv`` on its own GPU slot. The grid follows whatever that file
declares, and `cv` resumes at fold level, so re-running only recomputes the folds
whose output directory is missing.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--config", default="configs/cdr/cv.yaml")
...
def main() -> int:
    ...
    for task in tasks:
        for variant in base["variants"]:
            ...
            # Each job writes only its own condition; the root manifest is
            # assembled here once every job has finished.
    ...
    print(json.dumps({key: manifest[key] for key in
                      ("complete", "conditions_expected", "conditions_found")}))
    if failed:
