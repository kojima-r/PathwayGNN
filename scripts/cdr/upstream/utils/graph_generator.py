...
def convert_gene_ids(graph: DataFrame, ensambl: DataFrame) -> DataFrame:
    ensambl["HGNC ID"] = ensambl["HGNC ID"].str.split("HGNC:", n=1).str[-1]
...
def create_labels_relations_dic(
    ...
    # The sets are sorted before numbering: Python randomizes string hashing per
    # process, so enumerating them directly gave a different encoding on every
    # run, and any model trained against a previous vertices_dic.tsv no longer
    # matched the regenerated graph.
    def create_dic_nodes(graph_):
        ...
        return {v: n for n, v in enumerate(sorted(unique_nodes))}

    def create_dic_relations(graph_):
        uniq_r = graph_["relationship"].unique()
        unique_relationship = set(uniq_r)

        return {v: n for n, v in enumerate(sorted(unique_relationship))}
...
class GraphGeneration(object):
    ...
    def clean_graph(self) -> None:
        ...
        self.graph_data["A"] = (
            self.graph_data["A"].str.split("ENSEMBL:", n=1).str[-1]
        )
        self.graph_data["B"] = (
            self.graph_data["B"].str.split("ENSEMBL:", n=1).str[-1]
        )
